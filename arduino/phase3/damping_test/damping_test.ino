...
// ダンピング(角速度に比例した自動補正)のゲイン。値が大きいほど強く効く。
// 符号・軸の対応(gx→pitch, gy→roll)は仮定。実機テストで逆方向に効いてたら
// 符号を反転させる、または軸を入れ替える
const float DAMPING_GAIN = 1.5;
...
void setup() {
  ...
  Serial.println("=== RC-モーター連動 + IMUダンピングテスト開始 ===");
...
void loop() {
  ...
  if (currentState == DISARMED) {
    ...
    // ダンピング項：今の角速度(gx,gy)に比例した補正で、急な傾きを打ち消す方向に働く
